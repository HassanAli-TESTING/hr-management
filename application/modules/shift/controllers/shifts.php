...
{   
	...
	{   
		$this->permission->module('shift','read')->redirect();
	...
	{ 
		...
		#-------------------------------#
		$this->form_validation->set_rules('shift_name',display('shift_name'));
		#-------------------------------#
		if ($this->form_validation->run() === true) {
			...
			if ($this->Employees_model->shift_create($postData)) { 
				$this->session->set_flashdata('message', display('successfully_saved'));
			} else {
				$this->session->set_flashdata('exception',  display('please_try_again'));
			}
			redirect("shift/shifts/create_shift");
